...
<div x-data="alpineToasts"
    ...
    <template x-for="i in items" :key="i.id">
        <div x-show="true" x-transition:enter="transition ease-out duration-300"
             ...
             :class="i.type === 'success' ? 'bg-emerald-50 border-emerald-200 text-emerald-800' : 'bg-rose-50 border-rose-200 text-rose-800'"
             class="flex items-start gap-3 px-4 py-3 rounded-xl shadow-lg border backdrop-blur-sm">
            <i class="ph-fill text-lg shrink-0" :class="i.type === 'success' ? 'ph-check-circle text-emerald-600' : 'ph-warning-circle text-rose-600'"></i>
        ...
        Alpine.data('alpineToasts', () => ({
            ...
            init() {
                ...
                <?php if(session('success')): ?>
                    window.toast(<?php echo json_encode(session('success'), 15, 512) ?>, 'success');
                <?php endif; ?>
                <?php if(session('error')): ?>
                    window.toast(<?php echo json_encode(session('error'), 15, 512) ?>, 'error');
                <?php endif; ?>
            }
        }));
    });
...
<?php endif; ?>
<?php /**PATH C:\laragon\www\doug\PBE\laragon\sistema_autorizacao_escolar\projeto\resources\views/partials/toasts.blade.php ENDPATH**/ ?>