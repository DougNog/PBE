        ...
        <template x-for="aluno in resultados" :key="aluno.id">
            <div class="bg-white rounded-2xl p-5 shadow-soft hover:shadow-lg border border-slate-100/80 transition-all flex items-center gap-4 animate-slide-up">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <template x-if="aluno.tem_autorizacao">
                        <p class="text-[11px] text-emerald-700 mt-0.5 flex items-center gap-2">
                            <span x-show="aluno.horario_saida" class="flex items-center gap-1">
                                <i class="ph ph-clock"></i>
                                <span x-text="`Saída: ${aluno.horario_saida}`"></span>
                            </span>
                            <span class="flex items-center gap-1">
                                <i class="ph ph-warning"></i>
                                <span x-text="`${aluno.faltas} falta(s)`"></span>
                            </span>
                        </p>
                    </template>
                    <p class="text-[11px] text-slate-400 mt-0.5 truncate" x-show="aluno.responsaveis && !aluno.tem_autorizacao"
                ...
                <div class="shrink-0">
                    <template x-if="aluno.tem_autorizacao">
                        <a :href="aluno.url_confirmar"
                           class="px-4 py-2.5 bg-orange-500 hover:bg-orange-600 text-white font-semibold rounded-xl text-sm flex items-center gap-1.5 transition-colors shadow-sm">
                            <i class="ph-fill ph-sign-out"></i> Verificar saída
                        </a>
                    </template>
                    <template x-if="!aluno.tem_autorizacao">
                        <span class="px-4 py-2.5 bg-slate-100 text-slate-400 font-semibold rounded-xl text-sm flex items-center gap-1.5 cursor-not-allowed select-none">
                            <i class="ph-fill ph-lock-key"></i> Sem autorização
                        </span>
                    </template>
                </div>
            </div>
