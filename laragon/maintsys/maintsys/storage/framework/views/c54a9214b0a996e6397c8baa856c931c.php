...
<?php $component->withAttributes([]); ?>
    ...
    <style>
        ...
        :root{--ink:#0a0a0f;--surface:#f5f4f0;--card:#fff;--muted:#888;--border:#e8e6e1;--accent:#e05c1a;--red:#ff4c4c;}
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        .hero::before{content:'';position:absolute;inset:0;background:radial-gradient(ellipse 60% 80% at 85% -10%,rgba(224,92,26,.45) 0%,transparent 65%),radial-gradient(ellipse 35% 55% at -5% 110%,rgba(245,158,11,.25) 0%,transparent 60%);pointer-events:none;}
        ...
        .icon{width:40px;height:40px;border-radius:12px;background:#fff0e9;display:flex;align-items:center;justify-content:center;font-size:1.1rem;}
        ...
        .field input:focus{border-color:var(--accent);box-shadow:0 0 0 3px rgba(224,92,26,.12);background:#fff;}
        ...
        .btn-save:hover{filter:brightness(1.1);transform:translateY(-2px);box-shadow:0 10px 28px rgba(224,92,26,.35);}
    ...
    <div class="root">
        <div class="hero">
            <div class="hero-inner">
                <a href="<?php echo e(route('fornecedores.index')); ?>" class="back"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 18 9 12 15 6"/></svg>Fornecedores</a>
                <div class="divider"></div>
                <span class="hero-title">Editar Fornecedor</span>
            </div>
        </div>
        <div class="body">
            ...
            <form action="<?php echo e(route('fornecedores.update',$fornecedor->id)); ?>" method="POST">
                ...
                <div class="card">
                    <div class="card-head"><div class="icon">🏭</div><div><div class="ct">Editar fornecedor</div><div class="cs">#<?php echo e($fornecedor->id); ?> · <?php echo e($fornecedor->nome); ?></div></div></div>
                    <div class="card-body">
                        <div class="fg">
                            <div class="field"><label>Nome / Razão Social *</label><input type="text" name="nome" value="<?php echo e(old('nome',$fornecedor->nome)); ?>" required><?php $__errorArgs = ['nome'];
...
unset($__errorArgs, $__bag); ?></div>
                            <div class="fg c2">
                                <div class="field"><label>CNPJ</label><input type="text" name="cnpj" value="<?php echo e(old('cnpj',$fornecedor->cnpj)); ?>" maxlength="18" oninput="maskCnpj(this)"><?php $__errorArgs = ['cnpj'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?><span class="ferr"><?php echo e($message); ?></span><?php unset($message);
...
unset($__errorArgs, $__bag); ?></div>
                                <div class="field"><label>Telefone</label><input type="text" name="telefone" value="<?php echo e(old('telefone',$fornecedor->telefone)); ?>" maxlength="15" oninput="maskTel(this)"><?php $__errorArgs = ['telefone'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?><span class="ferr"><?php echo e($message); ?></span><?php unset($message);
...
unset($__errorArgs, $__bag); ?></div>
                            ...
                            <div class="field"><label>E-mail</label><input type="email" name="email" value="<?php echo e(old('email',$fornecedor->email)); ?>" placeholder="contato@empresa.com"><?php $__errorArgs = ['email'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?><span class="ferr"><?php echo e($message); ?></span><?php unset($message);
...
unset($__errorArgs, $__bag); ?></div>
            ...
            <div style="margin-top:.75rem;">
                <form action="<?php echo e(route('fornecedores.destroy', $fornecedor->id)); ?>" method="POST" onsubmit="return confirm('Excluir <?php echo e($fornecedor->nome); ?>?')">
                    <?php echo csrf_field(); ?> <?php echo method_field('DELETE'); ?>
                    <button type="submit" class="btn-del"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/></svg>Excluir fornecedor</button>
                </form>
            </div>
        </div>
    </div>
    <script>
        function maskCnpj(el){let v=el.value.replace(/\D/g,'').slice(0,14);v=v.replace(/(\d{2})(\d)/,'$1.$2');v=v.replace(/(\d{3})(\d)/,'$1.$2');v=v.replace(/(\d{3})(\d)/,'$1/$2');v=v.replace(/(\d{4})(\d{1,2})$/,'$1-$2');el.value=v;}
...
<?php endif; ?><?php /**PATH C:\laragon\www\confeccao\resources\views/fornecedores/edit.blade.php ENDPATH**/ ?>