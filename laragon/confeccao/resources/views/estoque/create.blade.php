    ...
    <div class="root">
        <div class="hero">
            <div class="hero-inner">
                <a href="{{ route('estoque.index') }}" class="btn-back">
        ...
        <div class="body">
            ...
            <form action="{{ route('estoque.store') }}" method="POST">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="grid">
                            <div class="field">
                                ...
                                <input type="text" name="nome" value="{{ old('nome') }}" placeholder="Ex: Estoque A" required>
                                @error('nome')<span class="field-error">{{ $message }}</span>@enderror
                            </div>
                            <div class="field">
                                <label>Produto <span>*</span></label>
                                <input type="text" name="produto" value="{{ old('produto') }}" placeholder="Ex: Calças Jeans" required>
                                @error('produto')<span class="field-error">{{ $message }}</span>@enderror
                            </div>
                            <div class="field">
                                <label>Quantidade <span>*</span></label>
                                <input type="number" name="quantidade" id="quantidade" value="{{ old('quantidade', 0) }}" min="0" required>
                                @error('quantidade')<span class="field-error">{{ $message }}</span>@enderror
                            </div>
                            <div class="field">
                                <label>Custo unitário (R$) <span>*</span></label>
                                <input type="number" name="custo" id="custo" value="{{ old('custo', '0.00') }}" min="0" step="0.01" required>
                                @error('custo')<span class="field-error">{{ $message }}</span>@enderror
                            </div>
                        </div>
                    ...
                    <div class="card-foot">
                        <a href="{{ route('estoque.index') }}" class="btn-cancel">Cancelar</a>
