...
<head>
    ...
    <style>
        :root {
            ...
            --border:rgba(255,255,255,0.07); --accent:#e63946;
            --accent2:#f4a261; --text:#f0eff4; --muted:#6b6b80; --success:#2ec4b6;
        }
        *{margin:0;padding:0;box-sizing:border-box;}
        ...
        .bc-art{width:120px;height:120px;border-radius:12px;margin:0 auto 12px;background:rgba(255,255,255,0.04);border:1px solid var(--border);display:flex;align-items:center;justify-content:center;overflow:hidden;}
        .bc-art img{width:100%;height:100%;object-fit:contain;image-rendering:pixelated;filter:drop-shadow(0 4px 12px rgba(0,0,0,0.5));}
    ...
    <div id="tab-cadastrar" class="section">
        <div class="form-layout">
            <div class="form-card">
                ...
                <div class="field">
                    <label>Nome</label>
                    <input type="text" id="f-nome" placeholder="ex: Volthorn" oninput="atualizarPreview()">
                </div>
                <div class="field">
                    <label>Tipo</label>
...
<script>
const tipoMap={'fire':'fire','fogo':'fire','water':'water','água':'water','grass':'grass','planta':'grass','electric':'electric','elétrico':'electric','poison':'poison','veneno':'poison','normal':'normal','flying':'flying','voador':'flying','bug':'bug','inseto':'bug','ground':'ground','terra':'ground','psychic':'psychic','psíquico':'psychic','rock':'rock','pedra':'rock','ice':'ice','gelo':'ice','dragon':'dragon','dragão':'dragon','ghost':'ghost','fantasma':'ghost','dark':'dark','sombrio':'dark','steel':'steel','aço':'steel','fairy':'fairy','fada':'fairy','fighting':'fighting','lutador':'fighting'};
const tipoEmoji={'fire':'🔥','fogo':'🔥','water':'💧','água':'💧','grass':'🌿','planta':'🌿','electric':'⚡','elétrico':'⚡','poison':'☠️','veneno':'☠️','normal':'⭐','flying':'🌪️','voador':'🌪️','bug':'🐛','inseto':'🐛','ground':'🌍','terra':'🌍','psychic':'🔮','psíquico':'🔮','rock':'🪨','pedra':'🪨','ice':'❄️','gelo':'❄️','dragon':'🐉','dragão':'🐉','ghost':'👻','fantasma':'👻','dark':'🌑','sombrio':'🌑','steel':'⚙️','aço':'⚙️','fairy':'✨','fada':'✨','fighting':'🥊','lutador':'🥊'};
const tipoColor={'fire':'#ff6b35','water':'#4da8ff','grass':'#5dbb37','electric':'#f5cc00','poison':'#b97fe8','normal':'#aaa','flying':'#89aef5','bug':'#a8bb1f','ground':'#c8a02c','psychic':'#ff4d8b','rock':'#c8a938','ice':'#98d8d8','dragon':'#9b6dff','ghost':'#9b7fd4','dark':'#a08070','steel':'#b8b8d0','fairy':'#f0a0c8','fighting':'#e05050'};
...
async function buscarPokemon(){
    ...
    try{const r=await fetch('/pokemon/'+q);const d=await r.json();if(!r.ok) return renderResultado(null,false,d.erro||'Não encontrado');renderResultado(d.resultados);}
...
async function pokemonAleatorio(){
    ...
    renderResultado(null,true);
    try{const r=await fetch('/pokemon/'+id);const d=await r.json();if(!r.ok) return renderResultado(null,false,'Erro ao buscar');renderResultado(d.resultados);}
    catch(e){renderResultado(null,false,'Erro de conexão');}
}
function renderResultado(p,loading=false,erro=null){
    ...
    const tipo=p.tipo||'Normal';const tc=getTipoColor(tipo);
    const num=String(p.identificador||'').padStart(3,'0');const s=p.identificador||50;
    const stats=[{label:'HP',val:Math.min(255,45+(s%80))},{label:'Ataque',val:Math.min(255,49+(s%90))},{label:'Defesa',val:Math.min(255,49+(s%70))},{label:'Velocidade',val:Math.min(255,45+(s%100))}];
    area.innerHTML=`<div class="poke-spotlight" style="--type-color:${tc}"><div class="poke-img-bg"><img src="${p.foto}" alt="${p.nome_do_pokemon}"></div><div class="poke-info"><div class="poke-number"># ${num}</div><div class="poke-name">${p.nome_do_pokemon}</div><div class="poke-types"><span class="type-badge ${getTipoClass(tipo)}">${getTipoEmoji(tipo)} ${tipo}</span></div><div class="poke-stats">${stats.map(st=>`<div class="stat-item"><label>${st.label}</label><div class="stat-bar-wrap"><div class="stat-bar" style="width:0%;background:${tc}" data-val="${st.val}"></div></div><div class="stat-val" style="color:${tc}">${st.val}</div></div>`).join('')}</div></div></div>`;
    requestAnimationFrame(()=>{document.querySelectorAll('.stat-bar').forEach(b=>{b.style.width=(parseInt(b.dataset.val)/255*100)+'%';});});
}
...
async function carregarBanco(){
    ...
    try{
        ...
        grid.innerHTML=d.pokemons.map((p,i)=>{
            const tc=getTipoColor(p.tipo);
            const arte=p.imagem
                ? `<img src="${esc(p.imagem)}" alt="${esc(p.nome)}">`
                : `<span style="font-size:3rem;line-height:120px">${getTipoEmoji(p.tipo)}</span>`;
            return `
            <div class="banco-card" style="animation-delay:${i*0.05}s">
                <div class="bc-id">#${String(p.id).padStart(3,'0')}</div>
                <div class="bc-art" style="border-color:${tc}30;background:${tc}10">${arte}</div>
                <div class="bc-name">${esc(p.nome)}</div>
                <span class="type-badge ${getTipoClass(p.tipo)}" style="font-size:0.68rem;padding:4px 12px">${esc(p.tipo)}</span>
                <div class="bc-atk">ATK <span>${p.ataque}</span></div>
                <button class="btn-delete" onclick="confirmarExclusao(${p.id},'${esc(p.nome)}')">× Excluir</button>
            </div>`;
        }).join('');
    }catch(e){grid.innerHTML='<div class="empty-state"><div class="icon">⚠️</div><p>Erro ao carregar dados</p></div>';}
}
