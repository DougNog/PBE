    ...
    <div class="root">
        <div class="hero">
            <div class="hero-inner">
                <a href="{{ route('fornecedores.index') }}" class="back"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 18 9 12 15 6"/></svg>Fornecedores</a>
        ...
        <div class="body">
            ...
            <form action="{{ route('fornecedores.update',$fornecedor->id) }}" method="POST">
                @csrf @method('PUT')
                <div class="card">
                    <div class="card-head"><div class="icon">🏭</div><div><div class="ct">Editar fornecedor</div><div class="cs">#{{ $fornecedor->id }} · {{ $fornecedor->nome }}</div></div></div>
                    <div class="card-body">
                        <div class="fg">
                            <div class="field"><label>Nome / Razão Social *</label><input type="text" name="nome" value="{{ old('nome',$fornecedor->nome) }}" required>@error('nome')<span class="ferr">{{ $message }}</span>@enderror</div>
                            <div class="fg c2">
                                <div class="field"><label>CNPJ</label><input type="text" name="cnpj" value="{{ old('cnpj',$fornecedor->cnpj) }}" maxlength="18" oninput="maskCnpj(this)">@error('cnpj')<span class="ferr">{{ $message }}</span>@enderror</div>
                                <div class="field"><label>Telefone</label><input type="text" name="telefone" value="{{ old('telefone',$fornecedor->telefone) }}" maxlength="15" oninput="maskTel(this)">@error('telefone')<span class="ferr">{{ $message }}</span>@enderror</div>
                            </div>
                            <div class="field"><label>E-mail</label><input type="email" name="email" value="{{ old('email',$fornecedor->email) }}" placeholder="contato@empresa.com">@error('email')<span class="ferr">{{ $message }}</span>@enderror</div>
                        </div>
                    </div>
                    <div class="card-foot">
            ...
            <div style="margin-top:.75rem;">
                <form action="{{ route('fornecedores.destroy', $fornecedor->id) }}" method="POST" onsubmit="return confirm('Excluir {{ $fornecedor->nome }}?')">
                    @csrf @method('DELETE')
