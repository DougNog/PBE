    ...
    <div class="root">
        <div class="hero">
            <div class="hero-inner">
                <a href="{{ route('estoque.index') }}" class="back"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 18 9 12 15 6"/></svg>Estoque</a>
        ...
        <div class="body">
            ...
            <form action="{{ route('estoque.update', $item->id) }}" method="POST">
                @csrf @method('PUT')
                <div class="card">
                    <div class="card-head"><div class="icon">📦</div><div><div class="ct">Editar item de estoque</div><div class="cs">#{{ $item->id }} · {{ $item->nome }}</div></div></div>
                    <div class="card-body">
                        <div class="fg">
                            <div class="fg c2">
                                <div class="field"><label>Nome do item *</label><input type="text" name="nome" value="{{ old('nome',$item->nome) }}" required>@error('nome')<span class="ferr">{{ $message }}</span>@enderror</div>
                                <div class="field"><label>Produto *</label><input type="text" name="produto" value="{{ old('produto',$item->produto) }}" required>@error('produto')<span class="ferr">{{ $message }}</span>@enderror</div>
                            </div>
                            <div class="fg c2">
                                <div class="field"><label>Quantidade *</label><input type="number" id="qty" name="quantidade" value="{{ old('quantidade',$item->quantidade) }}" min="0" required oninput="calc()">@error('quantidade')<span class="ferr">{{ $message }}</span>@enderror</div>
                                <div class="field"><label>Custo unitário (R$) *</label><input type="number" id="cst" name="custo" value="{{ old('custo',$item->custo) }}" min="0" step="0.01" required oninput="calc()">@error('custo')<span class="ferr">{{ $message }}</span>@enderror</div>
                            </div>
            ...
            <div style="margin-top:.75rem;">
                <form action="{{ route('estoque.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Excluir este item?')">
                    @csrf @method('DELETE')
